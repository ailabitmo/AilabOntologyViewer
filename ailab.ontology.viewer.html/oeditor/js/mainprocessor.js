...
var ontologyViewer = kiv.graphStuff.ontologyViewer({});
...
function startRequest() {
    ...
    function renderClassTree() {
        ...
        var listCreator = function (d) {
            var xxx = d3.select(this).append("o").text(getName(d, Oclasses) + ((d in Oclasses) ? " (" + Oclasses[d].count[0] + ")" : ""))
                    .on('mousedown.choosr', function (a) {
                        renderInstsList(a);
                    })
                    .on('mouseover.choosr', function (d) {
                        d3.select(this).attr('class', 'hover')
                    })
                    .on('mouseout.choosr', function (d) {
                        d3.select(this).attr('class', '')
                    })
                ;
            tooltiper.AddToolTip(xxx, function (d) {
                return d;
            }, {x: 30, y: -10, sdelay: 300, eduration: 0});
            ...
            for (var index in arrayOfChildren) {
                newUl.selectAll('li').data(arrayOfChildren,function (d) {
                    return d;
                }).enter().append('li').each(listCreator);
            }
        }
        d3.select("#classesNum").text("CLASSES: " + objToArrayKeys(processedElements).length + " found");
        var classContainer = d3.select("#classContainer").html("").selectAll("li").data(firstLevel, function (d) {
            return d;
        });
    ...
    function renderInstsList(classId) {
        ...
        processAllQueriesAndGetResult([allInstsQuery], endpoint, function (d) {
            ...
            var realNames = d3.range(0, vals.length).map(function (d) {
                return {name: getName(vals[d], instances), val: vals[d]}
            })
            ...
            var spans = d3.select("#instsContainer").html("").selectAll("li")
                .data(realNames, function (d, i) {
                    return d.val;
                })
                .enter().append("li").append("span").text(function (d) {
                    return d.name;
                })
                .on('mouseover.choosr', function (d) {
                    if (!anonym(d.val))d3.select(this).attr('class', 'hover')
                })
                .on('mouseout.choosr', function (d) {
                    if (!anonym(d.val))d3.select(this).attr('class', '')
                })
                .on('mousedown.choosr', function (a) {
                    if (!anonym(a.val))renderEditor(a.val);
                });

            tooltiper.AddToolTip(spans, function (d) {
                return d.val;
            }, {x: 30, y: -10, sdelay: 300, eduration: 0});
    ...
    function renderEditor(objectId) {
        ontologyViewer.render(objectId, getRequestToInstance(objectId));
    }
}
...
function getSomeObjectColor(objId, listWhereItExists) {
    ...
    return d3.hsl(Math.round(hslVal * 360), 0.5, 0.5).toString();
}
